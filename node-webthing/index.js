...
const { Action, Event, MultipleThings, SingleThing, Property, Thing, Value, WebThingServer } = require('webthing');
...
class ExampleDimmableLight extends Thing {
  constructor() {
    ...
    this.addAvailableAction(
      ...
      {
        title: 'AFade',
        description: 'AFade the lamp to a given level',
        input: {
          type: 'object',
...
class FakeGpioHumiditySensor extends Thing {
  constructor() {
    super('urn:dev:ops:my-humidity-sensor-1234', 'My Humidity Sensor', ['MultiLevelSensor'], 'A web connected humidity sensor');
...
function runServer() {
  ...
  const server = new WebThingServer(new MultipleThings([sensor, light], 'LightAndTemp'), 8888, 'Gerys-MacBook-Pro.local');
